...
function showMenu() {
    ...
    endpoints.forEach((ep, i) => {
        const index = i + 3;
        console.log(`\x1b[33m${index}\x1b[0m: ${ep.name} \x1b[90m${ep.path}\x1b[0m`);
    });
...
async function listUsers() {
    ...
    try {
        ...
        users.forEach(u => {
            console.log(`- \x1b[1m${u.email}\x1b[0m (Name: ${u.name}, Role: ${u.role})`);
        });
    } catch (e) {
...
async function testEndpoint(endpoint: typeof endpoints[0]) {
    ...
    try {
        ...
        else if (platform === 'linux') command = `echo '${curlCmd}' | xclip -selection clipboard || echo '${curlCmd}' | wl-copy`;

        if (command) {
            exec(command, (err) => {
                if (!err) console.log('\x1b[90m(Curl command copied to clipboard)\x1b[0m');
            });
        }
    } catch (err: any) {
