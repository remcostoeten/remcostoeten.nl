	...
	if (existsSync(configPath)) {
		try {
			...
			if (customConfig.endpoints)
				config.endpoints = customConfig.endpoints
			if (customConfig.githubUser)
				config.githubUser = customConfig.githubUser
		} catch (err) {
			console.log(
				`${c.yellow}Warning: Could not parse config file${c.reset}`
			)
		}
	}
}
...
async function checkGitHubUser(username) {
	...
	try {
		...
		if (res.status === 403) {
			return {
				exists: false,
				error: 'Rate limited. Add a GitHub token for more requests.'
			}
		}
...
async function fetchGhRepos() {
	return new Promise((resolve, reject) => {
		exec(
			'gh repo list --json name,description,isPrivate,stargazerCount,url --limit 100',
			(err, stdout, stderr) => {
				if (err) {
	...
	return new Promise(resolve => {
		...
		function filterRepos() {
			...
			selectedIndex = Math.min(
				selectedIndex,
				Math.max(0, filteredRepos.length - 1)
			)
		}
		...
		function render() {
			...
			console.log(
				`${c.dim}Type to search, arrows/j/k to navigate, Enter to select, q to cancel${c.reset}\n`
			)
			...
			if (filteredRepos.length === 0) {
				console.log(
					`${c.dim}  No repositories match your search${c.reset}`
				)
			}
			...
			for (let i = start; i < end; i++) {
				...
				const name = isSelected
					? `${c.bold}${repo.name}${c.reset}`
					: repo.name
				const stars =
					repo.stargazerCount > 0
						? ` ${c.yellow}*${repo.stargazerCount}${c.reset}`
						: ''
				const priv = repo.isPrivate
					? ` ${c.dim}[private]${c.reset}`
					: ''
				...
				if (isSelected && repo.description) {
					console.log(
						`    ${c.dim}${repo.description.substring(0, 55)}${repo.description.length > 55 ? '...' : ''}${c.reset}`
					)
				}
			}
			...
			drawSeparator(60)
			console.log(
				`${c.dim}${filteredRepos.length}/${repos.length} repos${c.reset}`
			)
		}
...
async function handleQueryUser(username) {
	...
	console.log(
		`  ${c.dim}Repos: ${u.publicRepos} | Followers: ${u.followers}${c.reset}`
	)
...
async function handleQueryRepo(repoPath) {
	if (!repoPath || !repoPath.includes('/')) {
		console.log(
			`${c.red}Error: Repository path required (format: owner/repo)${c.reset}`
		)
	...
	drawSeparator(60)
	console.log(
		`  ${c.bold}${r.fullName}${c.reset}${r.isPrivate ? ` ${c.yellow}[private]${c.reset}` : ''}`
	)
	if (r.description) {
	...
	console.log()
	console.log(
		`  ${c.yellow}Stars: ${r.stars}${c.reset}  |  Forks: ${r.forks}  |  ${r.language || 'No language'}`
	)
	if (r.topics.length > 0) {
		console.log(
			`  ${c.cyan}Topics: ${r.topics.slice(0, 5).join(', ')}${r.topics.length > 5 ? '...' : ''}${c.reset}`
		)
	}
	console.log(`  ${c.dim}Default branch: ${r.defaultBranch}${c.reset}`)
	console.log(
		`  ${c.dim}Updated: ${new Date(r.updatedAt).toLocaleDateString()}${c.reset}`
	)
...
async function runEndpoint(endpoint, silent = false) {
	...
	try {
		...
		if (
			...
			console.log(
				`${c.dim}Server not running at ${config.baseUrl}${c.reset}`
			)
...
async function runAllEndpoints() {
	...
	for (const endpoint of config.endpoints) {
		...
		console.log(
			`${statusIcon} ${status}  ${timing.padEnd(15)}  ${c.dim}${endpoint.name}${c.reset}`
		)
	}
	...
	drawSeparator()
	console.log(
		`${c.bold}Summary:${c.reset} ${c.green}${passed} passed${c.reset}${failed > 0 ? `, ${c.red}${failed} failed${c.reset}` : ''}`
	)
...
`)
	...
	config.endpoints.forEach((ep, i) => {
		...
		console.log(
			`      ${c.dim}${ep.method || 'GET'} ${ep.path}${c.reset}\n`
		)
	})
	...
	rl.question(`${c.bold}> ${c.reset}`, input =>
		handleInteractiveInput(input, rl)
	)
}
...
async function handleInteractiveInput(input, rl) {
	...
	if (cmd === 'r') {
		rl.question(
			`${c.cyan}Repository (owner/repo): ${c.reset}`,
			async repo => {
...
function showHistory() {
	...
	history.slice(0, 15).forEach(req => {
		...
		const statusIcon = req.success
			...
			: `${c.red}[ERR]${c.reset}`
		console.log(
			`${c.dim}${time}${c.reset}  ${statusIcon}  ${formatDuration(req.duration).padEnd(15)}  ${c.dim}${req.endpoint}${c.reset}`
		)
	})
}
