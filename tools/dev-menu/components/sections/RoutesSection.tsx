...
function RouteLink({ route, pathname }: Props & { route: RouteItem }) {
	...
	return (
		...
		>
			<span>{route.label}</span>
			{isActive && (
				<div className="w-1 h-1 bg-[hsl(167.8,53.25%,54.71%)]" />
			)}
		</Link>
	)
}
	...
	return (
		<div className="p-2">
			...
			<div className="text-[9px] text-[hsl(0,0%,40%)] px-2 py-1.5 mt-1 border-b border-[hsl(0,0%,18%)]">
				<span className="text-[hsl(0,0%,55%)]">@</span>{' '}
				<span className="text-[hsl(167.8,53.25%,65%)]">{pathname}</span>
			</div>

			<div className="space-y-1 max-h-[160px] overflow-y-auto scrollbar-hide mt-1">
				{(
					Object.entries(categorizedRoutes) as [
