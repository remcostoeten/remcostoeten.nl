...
export function useWidgetManager(): TWidgetManagerState &
	TWidgetManagerActions {
	...
	const addWidget = useCallback(
		(type: string, position: TWidgetPosition = { x: 100, y: 100 }): string => {
	...
	const updateWidget = useCallback(
		(id: string, updates: Partial<TWidgetConfig>) => {
			setState((prev) => ({
				...prev,
				widgets: {
					...prev.widgets,
	...
	const moveWidget = useCallback(
		(id: string, position: TWidgetPosition) => {
	...
	const updateDropZone = useCallback(
		(id: string, updates: Partial<TDropZone>) => {
			setState((prev) => ({
				...prev,
				dropZones: {
					...prev.dropZones,
	...
	const getWidgetsByZone = useCallback(
		(zoneId: string): TWidgetConfig[] => {
			const zone = state.dropZones[zoneId];
			if (!zone) return [];

			return zone.widgets
				.map((widgetId) => state.widgets[widgetId])
				.filter(Boolean);
		},
	...
	const getWidgetAtPosition = useCallback(
		(position: TWidgetPosition): TWidgetConfig | null => {
