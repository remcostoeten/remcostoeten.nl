...
export function createInitialState(): TPersistedState {
	return { version: 1, input: '', options: { ...DEFAULT_OPTIONS } }
}
...
export function loadPersistedState(): TPersistedState {
	...
	try {
		...
		console.warn('Could not read the JSON tool state', error)
		return initial
	}
}
...
export function savePersistedState(state: TPersistedState) {
	try {
		...
		console.warn('Could not persist the JSON tool state', error)
	}
}
