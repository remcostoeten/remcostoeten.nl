...
export function SignInButton() {
        ...
        async function fetchProviders() {
            try {
                ...
                if (data.providers.length === 0) {
                    setLoadError('Sign-in is not available right now. Please configure an OAuth provider.')
                }
            } catch (error) {
                if (!isActive) return
    ...
    function handleSignIn(provider: Provider) {
        ...
        signIn.social({
            ...
            setIsLoading(null)
            setAuthError('Sign-in failed. Please check your OAuth configuration or try another provider.')
