 ...
 */
export function SpotifyEnvGenerator({
	showGuide = true
}: SpotifyEnvGeneratorProps) {
		...
		<div className="my-8 rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden">
			<div className="px-6 py-4 border-b border-zinc-800 bg-zinc-900 flex items-center justify-between">
				<div className="flex items-center gap-3">
					<div className="w-8 h-8 rounded-none bg-green-500/10 flex items-center justify-center">
			...
			<div className="p-6 space-y-4">
				...
				<div className="relative mt-6">
					<div className="absolute top-3 right-3 z-10">
						<button
							onClick={handleCopy}
							className={`px-3 py-1.5 rounded-none text-[10px] font-bold uppercase flex items-center gap-1.5 transition-all ${
								copied
...
function InputField({
	...
	return (
		<div>
			...
			{hint && (
				<p className="text-[10px] text-zinc-500 mt-1.5 ml-1">{hint}</p>
			)}
		</div>
	)
}
...
function ValidationSummary({
	...
	return (
		<div
			className={`mt-4 p-4 rounded-xl border ${allRequiredValid ? 'bg-green-500/5 border-green-500/20' : 'bg-zinc-900 border-zinc-800'}`}
		>
			<div className="flex items-center gap-2 mb-3">
				...
				<span
					className={`text-xs font-semibold ${allRequiredValid ? 'text-green-400' : 'text-amber-400'}`}
				>
					{allRequiredValid
						? 'Ready to Copy'
