	...
	const handleExecute = useCallback(async () => {
		...
		try {
			const res = await fetch(
				`https://api.spotify.com${selectedEndpoint.path}`,
				{
					headers: {
			...
			if (!res.ok) {
				setError(
					`${res.status}: ${data.error?.message || 'Request failed'}`
				)
				...
				<div>
					...
					<div className="relative">
						...
						>
							<div className="flex items-center gap-3">
								<span
									className={`px-2 py-0.5 rounded text-[10px] font-bold border ${methodColors[selectedEndpoint.method]}`}
								>
							...
							<ChevronDown
								className={`w-4 h-4 text-zinc-500 transition-transform ${showEndpoints ? 'rotate-180' : ''}`}
							/>
						</button>
						...
						{showEndpoints && (
							<div className="absolute z-20 top-full left-0 right-0 mt-2 bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden shadow-2xl">
								{SPOTIFY_ENDPOINTS.map(endpoint => (
									...
									>
										<span
											className={`px-2 py-0.5 rounded text-[10px] font-bold border ${methodColors[endpoint.method]}`}
										>
											{endpoint.method}
										</span>
										<div>
