...
import { Highlighter, Palette, Type, Link as LinkIcon, Bold } from 'lucide-react'
...
export function RichTextEditor({ initialContent, onChange, placeholder = 'Start typing...' }: TProps) {
  ...
  const [customHighlightColor, setCustomHighlightColor] = useState('85 100% 75%')
  const [showCustomColorInput, setShowCustomColorInput] = useState(false)
  
  const editorRef = useRef<HTMLDivElement>(null)
  const toolbarRef = useRef<HTMLDivElement>(null)

  const updateSegments = useCallback((newSegments: ContentSegment[]) => {
    setSegments(newSegments)
    onChange(newSegments)
  }, [onChange])

  const handleTextChange = useCallback((event: React.FormEvent<HTMLDivElement>) => {
  ...
  const applyHighlight = useCallback((style: THighlightStyle) => {
  ...
  return (
    <div className="relative">
      ...
      <details className="mt-4">
        <summary className="text-xs text-muted-foreground cursor-pointer">Debug: Segment Data</summary>
