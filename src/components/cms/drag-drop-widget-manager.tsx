...
type TWidgetType = 'feedback-form' | 'contact-form' | 'time-display' | 'custom-text';
...
export default function DragDropWidgetManager({
  ...
  function createWidget(type: TWidgetType, position: { x: number; y: number }): TWidget {
  ...
  function updateWidget(id: string, updates: Partial<TWidget>) {
    ...
    onWidgetUpdate?.(widgets.map(w => w.id === id ? { ...w, ...updates } : w));
  }
    ...
    if (zoneId) {
      ...
      if (zone && zone.acceptedTypes.includes(widgetData.type)) {
        ...
        setDropZones(prev => prev.map(z => 
          z.id === zoneId 
            ? { ...z, widgets: [...z.widgets, widgetData] }
            : z
        ));
      }
    } else {
      ...
      {hoverZone && (
        <div className="absolute bottom-4 left-4 z-50 bg-card border border-border rounded-lg p-4 min-w-[200px]">
          ...
          <div className="space-y-1">
            {dropZones.find(z => z.id === hoverZone)?.acceptedTypes.map(type => (
              <Button
                key={type}
                variant="ghost"
                size="sm"
