...
export default function PageEditor({ page, onSave, onBack }: PageEditorProps) {
	...
	const handlePageInfoChange = (
		field: keyof Pick<Page, "title" | "slug" | "description" | "isPublished">,
	...
	const handleReorderBlocks = (newOrder: string[]) => {
		const reorderedBlocks = newOrder.map(
			(id) => editingPage.blocks.find((block) => block.id === id)!,
		);
		setEditingPage((prev) => ({
			...prev,
	...
	return (
		<div className="flex h-screen">
			{/* Editor Panel */}
			<div
				className={`${isPreviewMode ? "w-1/2" : "w-full"} flex flex-col border-r border-border`}
			>
				{/* Header */}
				...
				<div className="bg-card border-b border-border p-6">
					...
					<div className="mb-4">
						...
						<textarea
							...
							onChange={(e) =>
								handlePageInfoChange("description", e.target.value)
							}
							rows={2}
							className="w-full px-3 py-2 border border-input rounded-lg bg-background text-foreground focus:ring-2 focus:ring-ring focus:border-transparent"
						/>
					...
					<div className="flex items-center">
						<input
							...
							onChange={(e) =>
								handlePageInfoChange("isPublished", e.target.checked)
							}
				...
				<div className="flex-1 overflow-y-auto bg-background p-6">
					<Reorder.Group
						...
						values={editingPage.blocks.map((block) => block.id)}
						className="space-y-4"
					>
						{editingPage.blocks
