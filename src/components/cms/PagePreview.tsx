...
export default function PagePreview({ page }: PagePreviewProps) {
  const renderSegment = (segment: ContentSegment) => {
    switch (segment.type) {
      ...
      case 'project-card':
        ...
        return (
          <div key={segment.id} className="my-4 p-4 border border-border rounded-lg bg-card">
            <h4 className="font-semibold text-foreground mb-2">{projectData.title}</h4>
            <p className="text-muted-foreground text-sm mb-3">{projectData.description}</p>
  ...
  const renderBlock = (block: any) => {
    ...
    if (block.type === 'heading') {
      return (
        <h1 key={block.id} className="text-4xl font-light text-foreground mb-8 leading-tight">
          {content}
        </h1>
      );
    }
    ...
    return (
      <p key={block.id} className="text-lg text-muted-foreground leading-relaxed mb-6">
        {content}
      </p>
    );
  };
