...
export default function BlockEditor({ block, onChange, onDelete }: BlockEditorProps) {
  ...
  const handleSegmentChange = (segmentId: string, updatedSegment: ContentSegment) => {
    onChange({
      ...block,
  ...
  const handleAddSegment = (type: ContentSegment['type']) => {
    const newSegment: ContentSegment = {
      ...
      data: type === 'highlighted' ? { hslColor: '85 100% 75%' } : 
            type === 'link' ? { url: '#' } :
            type === 'project-card' ? { projectData: { title: '', description: '', technologies: [] } } :
            undefined
    };
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs text-muted-foreground">
            {block.content.length} segment{block.content.length !== 1 ? 's' : ''}
          </span>
        </div>
        <button
          onClick={onDelete}
