...
interface StaggerContextValue {
    ...
    registerLayer: (id: string, element: HTMLElement, group: string, priority: number) => void;
...
export function StaggerProvider({ children, config: userConfig }: StaggerProviderProps) {
    ...
    const getSortedLayers = useCallback((group?: string): LayerData[] => {
    ...
    const getLayerIndex = useCallback((id: string): number => {
    ...
    const getLayerDelay = useCallback((id: string): number => {
    ...
    const markGroupVisible = useCallback((group: string) => {
    ...
    const value = useMemo<StaggerContextValue>(() => ({
        ...
        isGroupVisible,
        markGroupVisible,
    }), [config, registerLayer, unregisterLayer, getLayerDelay, getLayerIndex, isGroupVisible, markGroupVisible]);
...
export function useStaggerLayer<T extends HTMLElement = HTMLElement>(
    ...
    const isReady = disabled
        ? true
        : (shouldReduceMotion ? true : (isGroupVisible(group) && isInView));
...
export function enableStaggerDebug() {
    if (typeof window !== 'undefined') {
        ...
        console.log('[Stagger] Debug mode enabled. Refresh to see registration logs.');
    }
}
