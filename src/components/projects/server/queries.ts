...
export async function getProjects(includeHidden = false) {
  try {
    const query = includeHidden
      ...
      : db.select().from(projects).where(eq(projects.hidden, false)).orderBy(asc(projects.idx))
...
export async function getSettings() {
  try {
    const [settings] = await db.select().from(projectSettings).where(eq(projectSettings.id, "singleton"))

    if (!settings) {
      const [newSettings] = await db.insert(projectSettings).values({ id: "singleton", showN: 6 }).returning()
      return newSettings
    }
