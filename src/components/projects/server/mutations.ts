...
export async function createProject(data: {
	...
	const maxIdx =
		allProjects.length > 0 ? Math.max(...allProjects.map(p => p.idx)) : 0
...
export async function reorderProject(
	...
	if (newIdx > oldIdx) {
		await db
			...
			.set({ idx: sql`${projects.idx} - 1` })
			.where(
				sql`${projects.idx} > ${oldIdx} AND ${projects.idx} <= ${newIdx}`
			)
	} else {
		await db
			.update(projects)
			.set({ idx: sql`${projects.idx} + 1` })
			.where(
				sql`${projects.idx} >= ${newIdx} AND ${projects.idx} < ${oldIdx}`
			)
	}
...
export async function moveProject(
	...
	const [swapProject] = await db
		.select()
		.from(projects)
		.where(eq(projects.idx, targetIdx))
	...
	await db
		.update(projects)
		.set({ idx: project.idx })
		.where(eq(projects.id, swapProject.id))
	await db
		.update(projects)
