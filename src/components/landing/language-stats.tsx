	...
	useEffect(() => {
		const fetchLanguages = async () => {
			try {
				...
				const totalRepos = Array.from(languageMap.values()).reduce(
			...
			<div className="space-y-4 pt-4 pb-8 px-4">
				...
				<div className="flex flex-wrap gap-3 pt-2">
					{[1, 2, 3, 4, 5, 6].map(i => (
						<div
							key={i}
							className="h-9 w-28 bg-muted/20 rounded-lg animate-pulse"
						/>
					))}
				</div>
			</div>
	...
	return (
		<div className="space-y-5 pt-4 pb-6 px-4">
			<p className="prose-sm text-muted-foreground/80 leading-relaxed max-w-xl">
				Beyond my primary stack, I'm constantly exploring new paradigms
				and languages. This breakdown reflects my activity across{' '}
				<span className="text-foreground/90 font-medium">GitHub</span>,
				from production tools to experimental playgrounds.
			</p>

			<div className="flex flex-wrap gap-2.5">
				{languages.map((lang, index) => {
					const meta = LANGUAGE_META[lang.name] || {
						icon: SiTypescript,
						color: '#888'
					}
