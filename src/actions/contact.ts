...
import { contactSubmissions, contactInteractions, contactAbandonments } from '@/server/db/schema'
import { z } from 'zod'
...
export async function getContactAnalytics() {
    try {
        ...
        const avgTimeToAbandon = abandonmentTimes.length > 0
            ? Math.round(abandonmentTimes.reduce((sum, record) => sum + (record.timeToAbandon || 0), 0) / abandonmentTimes.length)
            : 0

        // Get abandonment by time ranges
        const abandonmentRanges = {
            under5s: abandonmentTimes.filter(t => (t.timeToAbandon || 0) < 5000).length,
            under15s: abandonmentTimes.filter(t => (t.timeToAbandon || 0) >= 5000 && (t.timeToAbandon || 0) < 15000).length,
            under30s: abandonmentTimes.filter(t => (t.timeToAbandon || 0) >= 15000 && (t.timeToAbandon || 0) < 30000).length,
            over30s: abandonmentTimes.filter(t => (t.timeToAbandon || 0) >= 30000).length,
        }
        ...
        return {
            ...
            data: {
                ...
                avgTimeToAbandon,
                abandonmentRanges,
                conversionRate: totalFormStarts[0].count > 0
                    ? Math.round((totalSubmissions[0].count / totalFormStarts[0].count) * 100)
                    : 0,
                clickToStartRate: totalClicks[0].count > 0
