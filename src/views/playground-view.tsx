...
import { getEnabled, CATEGORY_META, PLAYGROUND_CONFIG } from '@/core/playground'
import type {
	PlaygroundCategory as CategoryType,
	RegistryEntry
		...
		<div className="border-b border-border/50">
			<div className="px-4 py-6">
				...
				<p className="text-sm text-muted-foreground/70 leading-relaxed max-w-xl">
					A collection of components, utilities, and experiments.
					These artifacts are intentionally small or experimental and
					do not warrant their own repositories. Browse, copy, and
					adapt as needed.
				</p>
			</div>
		</div>
...
function PlaygroundCard({
	...
	const PreviewComponent =
		'preview' in entry && entry.preview
			? (entry.preview as ComponentType)
			: null
	const FullComponent =
		'component' in entry && entry.component
			? (entry.component as ComponentType)
			: null
	...
	return (
		<div className="py-16 text-center">
			<p className="text-sm text-muted-foreground/60">
				No items enabled. Edit{' '}
				<code className="text-xs bg-muted/30 px-1.5 py-0.5">
					config.ts
				</code>{' '}
				to enable entries.
			</p>
