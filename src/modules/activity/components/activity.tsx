...
import { ActivityShell } from "./shell"
 ...
 *
 * Fixed height containers prevent layout shift across all states
 */
...
export function LatestActivity() {
  ...
  const loadActivities = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch('/api/activity/github')
  ...
  return (
    <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <ActivityShell githubContent={githubContent} spotifyContent={<SpotifyIntegration />} />
    </div>
  )
}
