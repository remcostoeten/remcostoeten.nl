...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`GitHub API error for ${owner}/${repo}: ${response.status} ${response.statusText}. Rate limit remaining: ${rateLimitRemaining}`);
            return NextResponse.json(
                { error: `GitHub API error: ${response.statusText}`, status: response.status },
