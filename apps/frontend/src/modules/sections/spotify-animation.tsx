...
export const SpotifyAnimation = () => {
  ...
  const displayTrack =
    currentlyPlaying || (recentTracks.length > 0 ? recentTracks[currentTrackIndex] : FALLBACK_SONGS[0])
        ...
        <PopoverContent className="w-80 p-0" align="start">
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-medium text-sm">{isPlaying ? "Currently Playing" : "Recent Tracks"}</h3>
              {useRealSpotify && <span className="text-xs text-green-500">🎵 Live</span>}
            </div>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {allTracks.slice(0, 5).map((track, index) => {
                ...
                return (
                  ...
                  >
                    {imageUrl ? (
                      <img src={imageUrl} alt={trackAlbum} className="w-10 h-10 rounded object-cover flex-shrink-0" />
                    ) : (
            ...
            {useRealSpotify && (
              <div className="mt-3 pt-3 border-t border-border">
                <p className="text-xs text-muted-foreground text-center">Powered by Spotify • Updates every 30s</p>
              </div>
            )}
          </div>
        </PopoverContent>
